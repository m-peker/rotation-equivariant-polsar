"""Exp. 31 - the main table again, at ten seeds instead of three.

A reviewer objects, correctly, that a paper criticising the statistical
standards of its field should meet a higher one itself: three seeds do not
support the differences we discuss. This is Exp. 15 unchanged except for the
seed count, written to its own store so the three-seed numbers remain
...
# Ten seeds on Flevoland only. The differences the paper actually argues
# about are on Flevoland, and ten seeds x three scenes x six arms does not
# fit the compute available. The other two scenes keep the three-seed
# numbers of Exp. 15, and the tables say so.
for sc in ("flevoland",):
    todo = [a for a in ARMS
            if True   # CV-MsAtViT artik UC sahnede de kosuyor (hakem: en guclu
                      # temel yontem neden tek sahnede?)
